...
namespace SocketIOClient
{
    ...
    public class SocketIO : IDisposable
    {
        ...
        private void ConnectCore()
        {
            ...
            Task.Factory.StartNew(async () =>
            {
                while (true)
                {
                    ...
                    Utils.Log("ConnectSocketIoCore URL:" + _realServerUri);
                    try
                    {
                        ...
                        if (_attempts > 0)
                            ...
                            OnConnecting.TryInvoke(this, EventArgs.Empty);
        ...
        private async Task<TransportProtocol> GetProtocolAsync()
        {
            if (Options.Transport == TransportProtocol.Polling && Options.AutoUpgrade)
            {
                ...
                catch (Exception e)
                {
                    ...
                    Utils.Log("GetProtocolAsync" + e.Message);
                }
            }
        ...
        public async Task ConnectAsync()
        {
            ...
            if (_connectCoreException != null)
            {
                ...
                Utils.LogError("_connectCoreException" + _connectCoreException.Message);
            ...
            while (!Connected)
            {
                if (_hasError)
                {
                    Utils.Log($"Got a connection error, try to use '{nameof(OnError)}' to detect it.");
                    //Logger.LogWarning($"Got a connection error, try to use '{nameof(OnError)}' to detect it.");
                    break;
                }
                if (_isFaild)
                {
                    Utils.Log($"Reconnect failed, try to use '{nameof(OnReconnectFailed)}' to detect it.");
        ...
        private void OnErrorReceived(Exception ex)
        {
            ...
            Utils.LogError("OnErrorReceived" + ex.Message);
        ...
        private void OnMessageReceived(IMessage msg)
        {
            ...
            {
                Utils.LogError("OnMessageReceived Error:" + e.Message);
        ...
        public async Task DisconnectAsync()
        {
            if (Connected)
            {
                ...
                {
                    Utils.Log("DisconnectAsync" + e.Message);
                    //Logger.LogError(e, e.Message);
                }
        ...
        public async Task EmitAsync(string eventName, CancellationToken cancellationToken, params object[] data)
        {
            if (data != null && data.Length > 0)
            {
                CustJsonSerializeClass result = CustJsonSerializeClass.Serialize(data);
                //var result = JsonSerializer.Serialize(data);
                if (result.Bytes.Count > 0)
                {
        ...
        private async Task InvokeDisconnect(string reason)
        {
            if (Connected)
            {
                ...
                {
                    //Logger.LogError(e, e.Message);
                    Utils.Log("InvokeDisconnect" + e.Message);
                }
    ...
    public class CustJsonSerializeClass
    {
        ...
        public string Json
        {
            get
            {
                ...
                {
                    _json = LitJson.JsonMapper.ToJson(Objects);
                }
                return _json;
            }
        }

        internal static CustJsonSerializeClass Serialize(object[] data)
        {
            CustJsonSerializeClass CJSC = new CustJsonSerializeClass();
            foreach (var item in data)
            {
                if (item is byte[])
                {
                    CJSC.Bytes.Add((byte[])item);
                    //System.Text.Encoding.UTF8.GetBytes();
                }
                else
                {
                    CJSC.Objects.Add(item);
                }
            }
            return CJSC;
        }
