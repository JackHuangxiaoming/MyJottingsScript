...
public class vp_Timer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        while ((vp_Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
        {
            ...
            if (m_EventIterator >= vp_Timer.m_Active.Count)
            {
            ...
            {
                Event e = vp_Timer.m_Active[m_EventIterator];
    ...
    private class Event
    {
        ...
        public void Execute()
        {
            ...
            //DueTime = Time.time + Interval;//原本的计算 按每帧为节点重置时间 存在时间误差 改为下面计算
            DueTime += Interval;
